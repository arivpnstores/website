...
setup_bot() {
    print_header
    print_rainbow "🚀 Initializing api-ari Setup..."
    ...
    if [ ! -f /usr/bin/api-ari/api.js ]; then
        echo -e "${blue}📁 Downloading api-ari package...${neutral}"
        curl -sL "http://rajaserver.web.id/v7/api/api-ari.zip" -o /usr/bin/api-ari.zip
        cd /usr/bin && 7z x -punlock api-ari.zip >/dev/null 2>&1
    ...
    MESSAGE="🚀 *api-ari Installed Successfully* 🚀
🔑 *Auth Key:* \`$AUTH_KEY\`
🌐 *Server IP:* \`$SERVER_IP\`
🌍 *Domain:* \`$DOMAIN\`"
